...
from readers.models import GDP, StackOverflow
...
class CountryGDPYoungestAge(APIView):
    ...
    def post(self, request):
        ...
        gdp = GDP.objects.filter(country_iso=country_iso_code.upper()).first()
        if not gdp:
            return HttpResponseBadRequest()
        min_age = StackOverflow.objects.filter(country_name=gdp.country_name).aggregate(min_age_total=Min('min_age_first_code'))['min_age_total']
